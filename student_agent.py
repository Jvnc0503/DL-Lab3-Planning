...
class AssemblyAgent:
    ...
    def _best_first(self, s0, key_fn, goal_fn, succ_fn, h_fn, max_depth=30, max_exp=90000):
        ...
        while frontier and expansions < max_exp:
            f, g, _, state, plan = heapq.heappop(frontier)
            key = key_fn(state)
    ...
    def _parse_blocks(self, scenario_context: str):
        ...
        init_facts = self._split_facts(initial_text)
        goal_facts = self._split_facts(goal_text)
        blocks = set(re.findall(r"the\s+([a-z]+)\s+block", scenario_context.lower()))
        ...
        return state, goal, sorted(blocks)
    ...
    def _blk_succ(self, state, goal, blocks):
        ...
        if holding is not None:
            x = holding
            preferred = [y for gx, y in goal["on"] if gx == x and y in clear]
            others = [y for y in sorted(clear) if y != x and y not in preferred]

            for y in preferred + others:
    ...
    def _plan_blocks(self, scenario_context: str) -> list[str]:
        ...
        return self._best_first(
            ...
            h_fn=lambda s: len(goal["on"] - s["on"]) + len(goal["ontable"] - s["ontable"]) + (1 if s["holding"] else 0),
    ...
    def solve(self, scenario_context: str, llm_engine_func) -> list:
        ...
        if is_blocks:
            ...
            raw = llm_engine_func(prompt=prompt, system=self.system_prompt_blocks, max_new_tokens=300, temperature=0.0, do_sample=False)
            return self._extract_actions(raw, self._blk_patterns)
        ...
        raw = llm_engine_func(prompt=prompt, system=self.system_prompt_objects, max_new_tokens=220, temperature=0.0, do_sample=False)
